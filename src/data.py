"""Dataset loading contract (TMDB pre-release success — next phase).

Implement ``load_dataset_split`` so ``scripts/main.py`` can evaluate models on
the same test split. Processed features must exclude post-release leakage; the
target ``movie_success_class`` is derived from ROI = revenue / budget only for
labeling (flop / average / hit).
"""
